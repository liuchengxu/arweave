...
test_block_validation() ->
	...
	BlockTXPairs0 = ar_node:get_block_txs_pairs(),
	TX =
	...
	?assertEqual(valid, validate(BI0, B, B0, Wallets, BlockTXPairs0)),
	?assertEqual(
		{invalid, invalid_height},
		validate(BI0, B#block{ height = 2 }, B0, Wallets, BlockTXPairs0)
	),
	?assertEqual(
		{invalid, invalid_weave_size},
		validate(
	...
	?assertEqual(
		{invalid, invalid_previous_block},
		validate(BI0, B#block{ previous_block = B#block.indep_hash }, B0, Wallets, BlockTXPairs0)
	),
	?assertEqual(
		{invalid, invalid_difficulty},
		validate(BI0, B#block{ diff = B0#block.diff - 1 }, B0, Wallets, BlockTXPairs0)
	),
	?assertEqual(
		{invalid, invalid_independent_hash},
		validate(BI0, B#block{ tags = [{<<"N">>, <<"V">>}] }, B0, Wallets, BlockTXPairs0)
	),
	?assertEqual(
		{invalid, invalid_wallet_list},
		validate(BI0, B#block{ txs = [TX#tx{ reward = ?AR(201) }] }, B0, Wallets, BlockTXPairs0)
	),
	...
	BlockTXPairs1 = ar_node:get_block_txs_pairs(),
	ar_node:mine(),
	...
	?assertEqual(valid, validate(BI1, B2, B, Wallets, BlockTXPairs1)),
	?assertEqual(
		{invalid, invalid_spora},
		validate(BI1, B2#block{ poa = #poa{} }, B, Wallets, BlockTXPairs1)
	),
	?assertEqual(
		{invalid, invalid_spora_hash},
		validate(BI1, B2#block{ hash = <<>> }, B, Wallets, BlockTXPairs1)
	),
	?assertEqual(
		{invalid, invalid_spora_hash},
		validate(BI1, B2#block{ hash = B#block.hash }, B, Wallets, BlockTXPairs1)
	),
	PoA = B2#block.poa,
	FakePoA =
		case get_chunk(1) of
	...
	?assertEqual(
		{invalid, invalid_spora},
		validate(BI1, B2#block{ poa = FakePoA }, B, Wallets, BlockTXPairs1)
	).
